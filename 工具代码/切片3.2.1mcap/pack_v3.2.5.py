...
【功能说明】
本工具用于将指定数据ID的数据进行切片处理（PACK、MCAP），并支持自定义存储位置。
...
class DataSlicerApp:
    def __init__(self):
        ...
        # 版本号 - 必须与远程 Auto_pack.sh 中的 SCRIPT_VERSION 一致
        self.REQUIRED_SCRIPT_VERSION = "3.2.5"
        self.VERSION = "V3.2.5"
        ...
        self._storage_var_trace_id = None
        self._invalid_ids = []
        self._current_nas_dir = None
    ...
    def execute_remote_script(self):
        try:
            ...
            if bolepack_path:
                ...
                # bolepack 未找到，使用默认环境配置，PACK数据将无法切分
                self.update_status("⚠️ 未找到bolepack程序，PACK格式数据将无法切分", "warning")
                lib_paths = [
                    "/home/user/.local/bolepack/lib",
                    "/home/user/.local/lib",
                    "/home/user/Fkm/pack/lib",
            ...
            stdin, stdout, stderr = ssh.exec_command(f"test -f {remote_script_path} && echo 'exists'")
            script_exists = stdout.read().decode().strip() == 'exists'
            
            if not script_exists:
                # 脚本不存在，先部署
                self.update_status("⚠️ 切分程序不存在，正在部署...", "warning")
                ...
                for retry in range(3):
                    try:
                        ...
                        if retry < 2:
                            self.update_status(f"⚠️ 部署重试 ({retry+1}/3): {e}", "warning")
                            time.sleep(1)
                        else:
                            raise Exception(f"脚本部署失败: {e}")
                if not deploy_success:
                    raise Exception("脚本部署失败，请检查网络连接")
            
            # 检查版本号
            self.update_status("正在检查脚本版本...", "processing")
            stdin, stdout, stderr = ssh.exec_command(f"grep '^SCRIPT_VERSION=' {remote_script_path} 2>/dev/null | cut -d'\"' -f2")
            remote_version = stdout.read().decode().strip()
            
            need_update = False
            if not remote_version:
                self.update_status("⚠️ 无法获取远程脚本版本，将重新部署...", "warning")
                need_update = True
            elif remote_version != self.REQUIRED_SCRIPT_VERSION:
                self.update_status(f"⚠️ 版本不匹配 (远程: {remote_version}, 需要: {self.REQUIRED_SCRIPT_VERSION})，正在更新...", "warning")
                need_update = True
            else:
                self.update_status(f"✅ 程序版本匹配: {remote_version}", "success")
            ...
            if need_update:
                ...
                for retry in range(3):
                    try:
                        ...
                        ssh.exec_command(f"chmod +x {remote_script_path}")
                        sftp.close()
                        self.update_status(f"✅ 脚本已更新至版本 {self.REQUIRED_SCRIPT_VERSION}", "success")
                        update_success = True
                        break
                    except Exception as e:
                        if retry < 2:
                            self.update_status(f"⚠️ 更新重试 ({retry+1}/3): {e}", "warning")
                            time.sleep(1)
                        else:
                            self.update_status(f"❌ 脚本更新失败: {e}", "error")
                            ssh.close()
                            return
                if not update_success:
                    self.update_status("❌ 脚本更新失败，请检查网络连接", "error")
                    ssh.close()
                    return
            ...
            while True:
                ...
                # 检查是否有新的数据ID - 增加明确的前缀匹配防止误判
                # 只有当行内容明确表示正在处理某个ID时才提取
                if "正在运行处理命令" in cleaned_line: # 忽略pack命令本身的输出
                    pass
                elif "Input Data ID:" in cleaned_line or "正在处理数据ID:" in cleaned_line: 
            ...
            if slice_results:
                ...
                # 如果是自定义路径，处理文件传输
                if storage_option == "2" and not terminated:
                    self.update_status("开始传输数据到本地...", "processing")
                    sftp = ssh.open_sftp()
                    try:
                        # 获取远程目录中的所有文件
                        remote_files = []
            ...
            terminated_data = sum(1 for result in slice_results.values() if result['status'] == '已终止')
            
            if terminated:
                self.update_status(f"⚠️ 任务已终止，共{total_data}个数据，{completed_data}个完成，{terminated_data}个已终止", "warning", show_timestamp=False)
            elif completed_data == total_data:
    ...
    def terminate_task(self):
        """终止任务，在单独的线程中运行以避免阻塞UI"""
        def _terminate():
            self._terminate_flag = True
            self.update_status("🛑 正在终止任务...", "warning")
            
            # 立即关闭SSH连接，这将导致主线程中的 paramiko 读取操作抛出异常并退出
            ...
            # 使用新的连接去清理远程进程
            try:
                ssh_clean = paramiko.SSHClient()
                ssh_clean.set_missing_host_key_policy(paramiko.AutoAddPolicy())
                ssh_clean.connect(
                    self.ssh_config["hostname"],
                    username=self.ssh_config["username"],
                    password=self.ssh_config["password"],
                ...
                # 使用 pkill 杀死所有相关进程
                clean_cmd = "pkill -f 'bolepack|mcap-split|Auto_pack.sh'"
                ssh_clean.exec_command(clean_cmd)
                ssh_clean.close()
            except Exception:
                pass 
            ...
            if hasattr(self, '_worker_thread') and self._worker_thread.is_alive():
                # 最多等待3秒，如果还没结束也强制重置状态
                self._worker_thread.join(timeout=3.0) 
            
            # 无论如何，强制重置状态
            self.is_running = False
            self.app.after(0, self.restore_controls)
            self.update_status("✅ 任务已终止。", "success")
