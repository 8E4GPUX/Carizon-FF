...
create_links() {
    ...
    # 添加调试信息
    echo -e "${YELLOW}正在搜索目录: $original_dir${NC}"
    
    # 检查目录是否存在
    if [ ! -d "$original_dir" ]; then
        echo -e "${RED}错误：目录不存在: $original_dir${NC}"
        return 1
    fi
    
    # 列出目录内容
    echo -e "${YELLOW}目录内容：${NC}"
    ls -la "$original_dir" 2>/dev/null || echo -e "${RED}无法列出目录内容${NC}"
    
    # 查找包含 "pack_" 的目录
    echo -e "${YELLOW}查找包含 'pack_' 的目录...${NC}"
    local pack_dirs=$(find "$original_dir" -type d -name "*pack_*" 2>/dev/null)
    
    if [ -z "$pack_dirs" ]; then
        echo -e "${YELLOW}未找到包含 'pack_' 的目录，显示所有子目录：${NC}"
        find "$original_dir" -type d -maxdepth 1 2>/dev/null || echo -e "${RED}无法列出子目录${NC}"
        echo -e "${YELLOW}未找到有效数据包${NC}"
        return 0
    fi
