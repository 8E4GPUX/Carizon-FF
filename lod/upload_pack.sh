...
cleanup() {
    ...
    sleep 0.2
    [ $cleaned -eq 2 ] && echo -e "${GREEN}清理操作已完成！${NC}"
}
...
validate_input() {
    # 数据地址验证
    local path_retry=0
    valid_inputs=()   # 初始化为全局变量，这里不使用 local
    local total_retry=0
    # 删除以下行，以确保 data_id_map 使用全局变量
    # declare -A data_id_map  # 存储提取的数据ID与用户原始输入的映射
    while true; do
        ...
        for input_path in "${inputs[@]}"; do
            ...
            if [[ "$input_part" =~ ([A-Za-z0-9]{6}_[0-9]{8}-[0-9]{6}-[0-9]{3}) ]]; then
                ...
                echo -e "${YELLOW}输入格式错误: $input_part，剩余尝试次数: $((3 - total_retry))${NC}"
            fi
        done

        if [ ${#valid_inputs[@]} -gt 0 ]; then
            echo -e "${GREEN}识别到${#valid_inputs[@]}个符合要求的数据ID${NC}"
            break
        else
            ((path_retry++))
        ...
        while true; do
            ...
            if [[ -n "$user_input_dir" ]]; then
                ...
                if [ ! -d "$user_input_dir" ]; then
                    ...
                    mkdir -p "$user_input_dir" || { echo -e "${RED}无法创建目录: $user_input_dir${NC}"; exit 1; }
                fi
...
main() {
    ...
    for data_id in "${valid_inputs[@]}"; do
        ...
        # 构建执行命令
        local output_dir="${OUT_DIR}/${file_name}"  # 修改此处，输出目录为 out/原始输入数据名
    ...
    echo -e "\n${BLUE}====== 数据切分完成 ======${NC}"
    echo -e "${GREEN}本次共切分了${#data_storage_map[@]}个数据ID${NC}"
